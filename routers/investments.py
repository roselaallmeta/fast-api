...
async def get_investment_by_title(investment: Investment) -> Investment | None:
    query = """
    SELECT 
        id, 
        user_id, 
...
"""
    ...
    async with database.pool.acquire() as connection:
        row = await connection.fetchrow(query, investment.title)
    if row:
        return Investment(
            id=row["id"],
            user_id=row["user_id"],
...
WHERE id = $10)"""
    ...
    async with database.pool.acquire() as connection:
        await connection.execute(query, investment.user_id, 
investment.venture_id,
investment.title,
investment.amount,
investment.investment_type,
